...
public class NicenessDAO {
	...
	public int insertNiceness(String userCode,String postCode)
	{
		...
		try
		{
			...
			pstmt.setString(1,nicenessCode);//코드생성필요
	...
	public LinkedList<String> searchTodayPostCode(String today){
		...
		try {
			String sql = "select post_code,count,rownum from(select post_code,count(*) countfrom niceness_tbWHERE NICENESS_DATE=?'group by post_code) WHERE rownum<3";
			Connection conn = dbConnection.getConn();
			pstmt = conn.prepareStatement(sql);
