...
public class SimpleRecipeViewDAO {
	...
	public LinkedList<SimpleRecipeViewVO> searchFlookRecipeListByRecipeName(String keyword)
	{
		...
		try {
			...
			while(res.next()) {
				simpleRecipeVOList.add(new SimpleRecipeViewVO(res.getString("recipe_Code"), res.getString("post_Code"), res.getString("complete_Image"), res.getString("recipe_Name"), res.getString("RECIPE_DESCRIPTION"), res.getInt("like_Count"), res.getInt("post_Hits"),res.getString("country_category_name"),res.getString("intredient_category_name"),res.getString("kind_category_name"),res.getString("situation_category_name"),res.getString("method_category_name")));
			}
		} catch(Exception ex) {
			ex.printStackTrace();
		} finally {
	...
	public static void main(String[]args) {
		/*System.out.println(SimpleRecipeViewDAO.getInstance().searchFlookRecipeListByHashTag("½ºÆÔ").get(0));*/
		System.out.println(SimpleRecipeViewDAO.getInstance().searchFlookRecipeListByRecipeName("±èÄ¡").get(0));
	}
