...
public class WriteRecipeServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		RequestDispatcher rd=request.getRequestDispatcher("writeRecipe.jsp");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//RecipeVO recipeVO= new RecipeVO(null, completeImage, recipeName, recipeDescription, personNumber, cookingTime, cookingLevel)
		/*
		...
		for (Part part : parts) {
			if (part.getContentType() != null) {
				...
				if (fileName != null && !fileName.isEmpty()) {
					...
					{
						CookingVO cookingVO=new CookingVO(null, null, i, dir, cookingCaption[i-1]);
	...
	private String getFilename(Part part) {
		...
		for (String element : elements) {
			...
			if (element.trim().startsWith("name")) {
				...
				/*return name+"/"+element.substring(element.indexOf('=') + 1).trim()
						.replace("\"", "");*/
